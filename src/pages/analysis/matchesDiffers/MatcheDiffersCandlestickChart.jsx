...
const CandlestickChart = ({ ticks, simpleMode, symbol, targetDigit }) => {
  ...
  const [chartOptions, setChartOptions] = useState({
    ...
    tooltip: {
      custom: ({ series, seriesIndex, dataPointIndex, w }) => {
        ...
        const isMatch = lastDigit === targetDigit;
        return `
          <div style="padding: 8px; background: #fff; border: 1px solid #d9d9d9; border-radius: 4px;">
            ...
            <div><strong>Last Digit:</strong> ${lastDigit} (${isMatch ? 'Matches' : 'Differs'} ${targetDigit})</div>
          </div>
        `;
      },
    },
  ...
  useEffect(() => {
    ...
    try {
      ...
      const candlestickData = candles.map((candle) => ({
        ...
        fillColor: candle.lastDigit === targetDigit ? '#52c41a' : '#f5222d',
        strokeColor: candle.lastDigit === targetDigit ? '#52c41a' : '#f5222d',
      }));
  ...
  }, [ticks, simpleMode, symbol, intervalSeconds, lastSymbol, targetDigit]);
