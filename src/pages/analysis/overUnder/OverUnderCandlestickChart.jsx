...
const CandlestickChart = ({ ticks, simpleMode, symbol, barrier }) => {
      ...
      custom: ({ series, seriesIndex, dataPointIndex, w }) => {
        ...
        return `
          <div style="padding: 8px; background: #fff; border: 1px solid #d9d9d9; border-radius: 4px;">
            ...
            <div><strong>Last Digit:</strong> ${lastDigit} (${isOver ? 'Over' : 'Under'} ${barrier})</div>
          </div>
        `;
      },
    },
  ...
  useEffect(() => {
    ...
    try {
      ...
      const candlestickData = candles.map((candle) => ({
        ...
        fillColor: candle.lastDigit > barrier ? '#52c41a' : '#f5222d', // Color based on Over/Under
        strokeColor: candle.lastDigit > barrier ? '#52c41a' : '#f5222d',
      }));
  ...
  }, [ticks, simpleMode, symbol, intervalSeconds, lastSymbol, barrier]);
